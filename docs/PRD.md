...
- ❌ Passive consumers searching for help → ✅ Supported builders with active partnership
- ❌ Isolated learning hoping for the best → ✅ Guided journey with success advocacy
- ❌ Discovering opportunities by luck → ✅ Opportunities found and delivered proactively
...
**Measurable indicators:**
- **Time to competency**: 50% reduction (from 6-12 months → 3-6 months) to "production-ready"
...
- **UBRI ROI**: Better conversion from university programs → productive developers
...
**Validation needed:**
- Does the readiness matching actually work? (developer ready ↔ opportunity fit)
...
- **Chats as Delivery Points:**
  ...
  - Slack during work → Discord with community → WhatsApp on mobile
...
**Why it's novel:**
- Most chat tools are channel-specific (Slack bot ≠ Discord bot)
...
**What it is:**
Not just matching developer skills to opportunities, but matching **developer readiness × opportunity timing × preparation window**.
...
- Learn from outcomes (track suggestion → application → success)
...
**How it's different:**
- ❌ "I'm an assistant, ask me questions" → ✅ "I'm working to make you succeed"
- ❌ "Here are some docs" → ✅ "I found an opportunity perfect for you"
...
- Example: Start debugging in Slack during day → Continue conversation in WhatsApp on commute → Ask follow-up in Discord at night
...
6. ✅ Authenticated - session created and persisted
...
7. ✅ All channels now linked to same Guru identity
...
Guru: ✅ Now has enough to proactively suggest relevant DeFi grants, recommend advanced paths, skip beginner content
```
...
Dev: "DeFi I think"
Guru: ✅ Captured interest area organically, continues building profile slowly
```
...
- **Channel Adapter Layer:** Abstracts platform differences (Slack API ≠ Discord API ≠ WhatsApp API)
...
- **Matching Algorithm:**
  - **Skill match:** Opportunity requirements ↔ Developer capabilities
  - **Interest match:** Opportunity focus (DeFi, NFT) ↔ Developer interests
...
- **Continuous Learning:**
  ...
  - Measure success rates (suggested opportunity → application → success)
...
**Example Tone:**
- ✅ "Congrats on finishing your first XRPL project! 🎉 You're now ready for intermediate DeFi concepts. Want to dive in?"
- ❌ "Task completed. Next module available. Click here to proceed."
...
- **Discord:** Reactions for feedback (👍 helpful, 👎 not useful)
...
```
Home/Dashboard
├── Progress Overview (skills, projects, milestones)
├── Opportunity Pipeline (matched grants, hackathons, jobs)
├── Account Settings
│   ├── Profile & Goals
│   ├── Connected Channels (Slack, Discord, WhatsApp)
│   ├── Notifications & Preferences
│   └── Privacy & Data
└── Conversation History (searchable across all channels)
```
...
- **Proactivity Slider:** Low (only critical) ↔ High (all suggestions)
...
- System maintains knowledge matrix with dimensions:
  ...
  - **Levels** (columns): Básico, Intermediário, Avançado
- Each cell represents a competency (Topic × Level)
- Developer profile shows which cells are validated, which are pending, which are unknown
- Opportunities require specific cells (e.g., "DeFi Grant requires DeFi Protocols - Intermediário")

**FR-2.2: Competency Assessment Flow**
- **Self-Declaration:** Developer says "I already know X" → System marks as "pending validation"
...
- **Observational Assessment:** Questions developer asks reveal gaps (asking basic question → not advanced)
- **Project Evidence:** Developer shows code/repo → AI validates competencies demonstrated
...
- **Evidence-based:** "I see you built an NFT marketplace - that validates NFTs - Intermediário"
...
- System highlights "unlock opportunities" (e.g., "Validate DeFi - Intermediário to unlock 3 grants")
...
- System calculates match score (0-100) for each opportunity vs. each developer:
  - Skill match: 30 points (developer capabilities ↔ opportunity requirements)
  - Interest match: 25 points (opportunity focus ↔ developer interests)
...
- System only notifies if match score ≥70%
...
**FR-5.2: Cross-Channel Context Continuity**
...
**FR-5.3: Channel-Specific Adaptation**
...
### FR-6: Web Dashboard (Optional Interface)

**FR-6.1: Account Management**
...
**FR-7.1: Developer-Facing Analytics**
...
**FR-7.2: Ecosystem-Facing Analytics (Aggregated)**
...
- System measures success rate: suggested opportunity → application → success

### FR-8: Privacy & Data Management

**FR-8.1: Data Collection Transparency**
...
- **Requirement:** 95th percentile response time ≤2 seconds for chat interactions
- **Measurement:** From developer message received → Guru response sent
...
- **Requirement:** Initial page load ≤1 second (p95)
...
- **Measurement:** Opportunity detected → Message sent to channel
...
- **Measurement:** Query sent to RAG → Results received
- **Rationale:** Developer waiting for technical answer - fast response critical
- **Acceptance:** Cached frequent queries ≤100ms, fresh queries ≤1s
...
- **In Transit:**
  - TLS 1.3 for all API communication (client ↔ backend, backend ↔ Azure services)
...
- **Calculation:** 10K developers × avg 10 messages/day = 100K messages/day
...
- **Calculation:**
  - 10K developers × 500 messages/developer (6 months avg retention) × 1KB/message = 5GB
...
- **Processing:** Match 100 opportunities × 10K developers = 1M match calculations per day
